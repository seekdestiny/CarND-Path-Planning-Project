...
vector<double> Vehicle::choose_next_state() {
    ...
    for(int i = 0; i < possible_successor_states.size(); i++) {
      ...
      vector<double> trajectory = generate_trajectory(possible_successor_states[i]);

      // Calculate the cost of that trajectory
      cost = calculate_cost(possible_successor_states[i], trajectory);
      costs.push_back(cost);
      final_results.push_back(trajectory);
    }
